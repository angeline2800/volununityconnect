            ...
            <section class="Showcase">
              <div class="col-div-10">
                <div class="box-10">
                    ...
                    <div class="viewOpt">
                        <button style="border: 3px solid #011A3D;"><a href="listings.php">One Time Voluntary Work</button></a>
                        <a href="listings_recurring.php"><button>Recurring Voluntary Work</button></a>
                    </div>
                    ...
                    function sort(answer)
                    {
                        if(answer.value == "All")
                            ...
                            window.location="listings.php?request="+answer.value; 
                    }
                    </script>
                    ...
                    <?php
                        if (isset($_GET['request'])) 
                            ...
                            {
                                $query = "SELECT * FROM voluntaryWork WHERE userid = $userid AND workStatus = 1 AND workType = 'One Time' ORDER BY workDate DESC";
                            } 
                            else if ($statusFilter == 'Inactive') 
                            {
                                $query = "SELECT * FROM voluntaryWork WHERE userid = $userid AND workStatus = 0 AND workType = 'One Time' ORDER BY workDate DESC";
                            } 
                            else 
                            {
                                $query = "SELECT * FROM voluntaryWork WHERE userid = $userid AND workType = 'One Time' ORDER BY workDate DESC";
                            }
                    ...
                    ?>
                        ...
                        <?php
                            ...
                            {
                                // Check if the workDate is in the past
                                $currentDate = date('Y-m-d');
                                if ($row['workDate'] < $currentDate) 
                                {
                                    // If the workDate is in the past, update the workStatus to '0'
                                    $workId = $row['id'];
                                    $updateStatusSql = "UPDATE voluntaryWork SET workStatus = 0 WHERE id = $workId";
                                    mysqli_query($conn, $updateStatusSql);
                                    // Update the $row array to reflect the updated workStatus
                                    $row['workStatus'] = 0;
                                    // Display "Expired!" instead of the toggle button
                                    $toggleButton = "Expired!<br>Please update!";
                                }
                                else 
                                {
                                    // If the workDate is today or in the future, display the toggle button
                                    $toggleButton = '<input type="checkbox" ' . ($row['workStatus'] == 1 ? 'checked' : '') . ' onchange="toggleStatus(' . $row['id'] . ', this.checked ? 1 : 0)">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if ($toggleButton == "Expired!<br>Please update!") 
                                { ?>
                                    <span><?php echo $toggleButton; ?></span>
                            <?php } 
                                else 
                                { ?>
                                    <label class="switch">
                            ...
                            <td>
                                <a class = "update" href='editListing.php?id=<?php echo $row['id']; ?>'><button>Update</button></a>
                ...
                function toggleStatus(workId, newStatus) 
                {
                    ...
                    var workDate = '<?php echo $row['workDate']; ?>'; 
